...
def partition_data(dataset, num_partitions):
    ...
    for i in range(num_partitions):
        ...
        remapped_edge_index = torch.stack([
            torch.tensor([node_mapping[node.item()] for node in data.edge_index[0]]),
            torch.tensor([node_mapping[node.item()] for node in data.edge_index[1]])
        ], dim=0)

        partition_data = data.clone()
        partition_data.x = data.x[all_nodes]
        partition_data.edge_index = remapped_edge_index
        partition_data.train_mask = data.train_mask[all_nodes]
        partition_data.val_mask = data.val_mask[all_nodes]
        partition_data.test_mask = data.test_mask[all_nodes]
        partition_data.y = data.y[all_nodes]
        partition_data.num_classes = dataset.num_classes

        partition_data.owned_nodes_mask = owned_mask
        partition_data.redundant_nodes_mask = ~owned_mask
